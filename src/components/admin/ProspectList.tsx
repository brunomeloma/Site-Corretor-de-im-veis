...
import { MessageCircle, UserPlus, X, Trash2, CheckCircle, Circle } from 'lucide-react';
...
export default function ProspectList({ prospects, onAdd, onToggleContacted, onDelete }: ProspectListProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-slate-800">Prospecção de Clientes</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="De onde veio? Indicação? Instagram?"
              />
            </div>
            <button
              type="submit"
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-6 py-2.5 rounded-xl transition-colors"
            >
              <UserPlus className="h-4 w-4" />
              Adicionar
            </button>
            ...
            <div className="flex items-start gap-3">
              ...
              >
                {prospect.contacted ? (
                  ...
                  <Circle className="h-5 w-5 text-slate-300 hover:text-blue-400 transition-colors" />
                )}
              </button>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {!prospect.contacted && (
                      <a
                        href={generateProspectWhatsAppLink(prospect.phone, prospect.name, prospect.vehicle || 'veículo')}
                        target="_blank"
